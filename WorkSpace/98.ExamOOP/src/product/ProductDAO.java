...
public class ProductDAO {
	...
	public void buyItem() {
		...
		if(common.items[choiceMenu-1].getQty()==0) {
			...
			return;
		}else if(common.loginInfo.getMoney()>= common.items[choiceMenu-1].getPrice()) {
			System.out.println("구매완료.");
			common.loginInfo.setMoney(common.loginInfo.getMoney()-common.items[choiceMenu -1].getPrice());
...
//		for (ProductDTO dto :commom.items) {
//			foreach : 따로 변수를 하나 외부에 만들고 증감식 통해서 보여줘야함.
//		}
//	}
		for(int i=0; i<common.items.length; i++) {
			System.out.println((i+1)+"."+common.items[i].getName());
			System.out.println("("+common.items[i].getPrice()+"원)");
			System.out.println(" "+common.items[i].getQty()+"개 남음\r\n");
		}
	}
	// 상품정보 초기화
