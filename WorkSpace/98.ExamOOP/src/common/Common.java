package common;

import java.util.Scanner;

import member.MemberDTO;
import product.ProductDTO;

public class Common {
	private Scanner scan;
	public MemberDTO loginInfo;//로그인 성공시 배열(DAO)에 있는 정보 중 하나의 데이터를 저장해둠. 

	public ProductDTO[] items;//최초 메인 메소드에서 상품을 초기화해두고 올림.

	// 1. 숫자형태를 입력받아야할때
	// 1.1+메세지
	// 1.2 값체크
	// 2.문자열을 입력받아야할때



	public String getStrScanner(String msg) {
		System.out.println(msg);
		return getStrScanner(0);
	}
	
	public MemberDTO getLoginInfo() {
		return loginInfo;
	}

	public void setLoginInfo(MemberDTO loginInfo) {
		this.loginInfo = loginInfo;
	}

	public String getStrScanner(int checkValue) {
		while(true) {
			String value = scan.nextLine();
			if(value.length()>checkValue) {
				return value;
			}else {
				System.out.println(checkValue +"보다 큰 글자수 입력");
			}
		}
		
		
	}
	
	public int getIntScanner(int checkValueMin, int checkValueMax) {
		while(true) {
			int value = getIntSanner();
			if(value>=checkValueMin && value <= checkValueMax) {
				return value;
				
			}else {
				System.out.println(checkValueMin +"~"+ checkValueMax +"사이값 입력");
			}
		}
		
	}
	public int getIntScanner(int checkValue) {
		while(true) {
			int value = getIntSanner();
			if(value>checkValue) {
				return value;
				
			}else {
				System.out.println(checkValue +" 보다큰값");
			}
		}
		
	}
	public int getIntSanner() {
		while (true) {
			try {
				return Integer.parseInt(scan.nextLine());
			} catch (Exception e) {
				System.out.println("숫자만 입력");

			}
		}
	}
	// 통신열기
	public Common() {
		scan = new Scanner(System.in);

	}

	// 통신닫기(종료)
	public void endProgram() {

		if (scan != null)
			scan.close();

		scan = null;
	}

}
